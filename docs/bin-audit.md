...
| `cert-get` | Print leaf cert as PEM; `--download` writes `<domain>.pem` in cwd | read, net (write:cwd with `--download`) | yes | run ✔ | 0 | Fine. `cert-download` symlink replaced by `-d`/`--download` (2026-09-25). | fix: template, option instead of symlink | fixed |
