...
If an image genuinely has no meaningful health signal (e.g. a one-shot job, a `scratch`-based static binary used only as a build artifact), write `HEALTHCHECK NONE` with a one-line comment explaining why. The directive is then present and intentional rather than forgotten.
